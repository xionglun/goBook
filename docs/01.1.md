...
访问[下载地址][downlink]，32位系统下载go1.1.1.darwin-386.pkg，64位系统下载go1.1.1.darwin-amd64.pkg，双击下载文件，一路默认安装点击下一步，这个时候go已经安装到你的系统中，默认已经在PATH中增加了相应的`~/go/bin`,这个时候打开终端，输入`go`
...
访问[下载地址][downlink]，32位系统下载go1.1.1.linux-386.tar.gz，64位系统下载go1.1.1.linux-amd64.tar.gz，

假定你想要安装Go的目录为 `$GO_INSTALL_DIR`，后面替换为相应的目录路径。

解压缩`tar.gz`包到安装目录下：`tar zxvf go1.1.1.linux-amd64.tar.gz -C $GO_INSTALL_DIR`。
...
访问[下载地址][downlink]，32位系统下载go1.1.1.windows-386.msi，64位系统下载go1.1.1.windows-amd64.msi。双击打开下载的文件，一路按照默认点击下一步，这个时候go已经安装到你的系统中，默认安装之后已经在你的系统环境变量中加入了`c:/go/bin`，这个时候打开cmd，输入`go`
